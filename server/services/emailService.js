...
function orderDetails(order) {
  ...
  const cod = order.paymentMode === "COD"
    ? `<tr><td style="padding:7px 0;">Online Advance (10%)</td><td style="padding:7px 0;text-align:right;">${formatMoney(order.codAdvance)}</td></tr><tr><td style="padding:7px 0;">Pay on Delivery (90%)</td><td style="padding:7px 0;text-align:right;">${formatMoney(order.codRemainingOnDelivery ?? order.remaining)}</td></tr>`
    : "";
...
async function sendEmail({ to, subject, html }) {
  if (!to || !canSend()) {
    console.warn("Email notification skipped", {
      reason: to ? "email_not_configured" : "missing_recipient",
      subject
    });
    return false;
  }
  ...
  try {
    ...
    if (error) {
      console.error("Email notification failed", {
        subject,
        message: error.message || "Resend email error"
      });
      return false;
    }
...
function sendWelcomeEmail(user) {
  return sendEmail({
    to: user.email,
    subject: "Welcome to Haroon Stores",
    html: emailLayout({
      title: "Welcome to Haroon Stores",
      greeting: user.name,
      body: paragraph("Your customer account has been successfully created. You can now explore our collection of Kashmiri handcrafted products and place orders with us.")
    })
  });
}
...
function sendLoginNotificationEmail(user) {
  return sendEmail({
    to: user.email,
    subject: "Welcome Back to Haroon Stores",
    html: emailLayout({
      title: "Welcome Back",
      greeting: user.name,
      body: `${paragraph("You have successfully signed in to your Haroon Stores account.")}${paragraph(`Sign-in time: ${formatDate()}`)}`,
      actionText: "If you did not perform this sign-in, please secure your account."
    })
  });
}
...
function sendOrderConfirmationEmail(user, order) {
  return sendEmail({
    to: user.email,
    subject: `Order Confirmed — Haroon Stores #${order.id}`,
    html: emailLayout({
      title: "Your order has been received",
      greeting: user.name,
      body: `${paragraph("Thank you for your order. We will keep you updated as it moves through each stage.")}${orderDetails(order)}`
    })
  });
}
...
function sendAdminNewOrderEmail(user, order) {
  return sendEmail({
    ...
    html: emailLayout({
      ...
      body: `${paragraph(`Customer: ${user.name} (${user.email})`)}${user.phone ? paragraph(`Phone: ${user.phone}`) : ""}${paragraph(`Order status: ${order.status}`)}${orderDetails(order)}`,
      actionText: "Please review and process this order in the Haroon Stores admin panel."
    })
  });
}
...
function sendStatusEmail(user, order, title, message) {
  return sendEmail({
    ...
    html: emailLayout({
      title,
      greeting: user.name,
      body: `${paragraph(message)}${orderDetails(order)}`
    })
  });
}

const sendOrderProcessingEmail = (user, order) =>
  sendStatusEmail(
    user,
    order,
    "Your Order Has Been Processed",
    "Our team is now preparing your order. We will notify you when it is shipped."
  );

const sendOrderShippedEmail = (user, order) =>
  sendStatusEmail(
    user,
    order,
    "Your Order Has Been Shipped",
    "Good news! Your order is now on its way to you."
  );

const sendOrderDeliveredEmail = (user, order) =>
  sendStatusEmail(
    user,
    order,
    "Your Order Has Been Delivered",
    "Your order has been successfully delivered. We hope you enjoy your Kashmiri handcrafted products."
  );
...
const sendOrderCancelledEmail = (user, order) =>
  sendStatusEmail(
    ...
    order,
    "Order Cancelled",
    "Your order has been cancelled successfully. Any refund information will be reflected only when confirmed by Haroon Stores."
  );

const sendReturnRequestedEmail = (user, order) =>
  sendStatusEmail(
    user,
    order,
    "Return Request Received",
    `We have received your return request.${order.returnReason ? ` Reason: ${order.returnReason}` : ""} Our team will review it and update you once a decision has been made.`
  );

const sendReturnApprovedEmail = (user, order) =>
  sendStatusEmail(
    user,
    order,
    "Return Request Approved",
    "Your return request has been approved. Please follow the return details provided by Haroon Stores."
  );

const sendReturnRejectedEmail = (user, order) =>
  sendStatusEmail(
    user,
    order,
    "Return Request Update",
    "We have reviewed your return request. Unfortunately, it has been rejected."
  );

function sendAdminReturnRequestedEmail(user, order) {
  return sendEmail({
    to: process.env.ADMIN_NOTIFICATION_EMAIL,
    subject: `New Return Request — #${order.id}`,
    html: emailLayout({
      title: "New return request",
      body: `${paragraph(`Customer: ${user.name} (${user.email})`)}${paragraph(`Reason: ${order.returnReason || "Not provided"}`)}`,
      actionText: "Please review this request in the Haroon Stores admin panel."
    })
  });
}
